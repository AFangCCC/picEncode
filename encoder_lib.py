...
if __name__ == "__main__":
    # 读入图像
    image_path = r'C:\Users\afangccc\Desktop\excemple.jpg'
    src_img = imread(image_path)
    # SVD
    waitKey(0)
    # 利用自定义SVD图像压缩模块对图像进行压缩
    zip_image_by_svd(src_img, rate=0.8)
    ...
    writeBinImage(img_encode, image_path.replace('.jpg', '.bin'))

    # 读取编码的文件，得到二进制编码数据字符串
    img_read_code = readBinImage(
        image_path.replace(
            '.jpg', '.bin'), len(img_encode))
